...
It's common for an identity provider, such
...
This is a useful map for use of keys within a JWK Set outside of `github.com/golang-jwt/jwt/v5`.
...
### Preconditions: Acquire the JWK Set URL, JSON, or gather cryptographic keys (given keys)

A JWK Set URL is not required, one can be created directly from JSON with the
[`keyfunc.NewJSON`](https://pkg.go.dev/github.com/MicahParks/keyfunc/v2#NewJSON) function.

```go
// Get the JWK Set URL from an environment variable.
...
### Step 1: Create the JWK Set

Via HTTP:

```go
// Create the JWK Set from the resource at the given URL.
jwks, err := keyfunc.Get(jwksURL, keyfunc.Options{}) // See recommended options in the examples directory.
if err != nil {
	log.Fatalf("Failed to get the JWK Set from the given URL.\nError: %s", err)
}
```
...
if err != nil {
	log.Fatalf("Failed to create JWK Set from JSON.\nError: %s", err)
}
```
...
// Create the JWK Set from the HMAC key.
...
* A background refresh of the JWK Set keys can be performed.
    ...
    * A custom rate limit can be specified to prevent too many requests for a JWK Set refresh. For an example, please see
      the `examples/recommended_options` directory.
    * JWTs with a previously unseen `kid` can prompt an automatic refresh of the remote JWK Set resource. This should be
...
* A custom HTTP request factory can be provided to create HTTP requests for the remote JWK Set resource. For example, an
  HTTP header can be added to indicate a User-Agent.
* A custom HTTP response extractor can be provided to get the raw JWK Set JSON from the `*http.Response`. For example, the
...
* A copy of the latest raw JWK Set `[]byte` can be returned.
...
* The remote JWK Set resource can be refreshed manually using the `.Refresh` method. This can bypass the rate limit, if the
  option is set.
