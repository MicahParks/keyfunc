		...
		for _, tc := range testCases {
			...
			t.Run(fmt.Sprintf("token: %s", tc.token), func(t *testing.T) {
				...
				if err != nil && !tc.expectError {
					...
					// Fail the test with the unexpected error.
					t.Errorf("Failed to parse JWT.\nError: %s", err.Error())
					t.FailNow()
				}
			})
		}
